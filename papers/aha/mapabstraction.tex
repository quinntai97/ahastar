...
\par \indent
We thus construct an abstract \emph{multi-graph} in which each edge $e$ is annotated with a single clearance value $cv(e, c_{e})$ where $c_{e} \in C$.
Each $e \in E_{abs}$ is traversable by an agent $a$ iff:
$$ cv(e, c_{a}) \geq s_{a} : c_{e} \subseteq c_{a} \in C, s_{a} \in S$$
