...
%nodes on the perimeter of each empty rectangle. 
...
The first case is simple: adjacent nodes from a rectangle $R$ are connected just
as in the original grid.  In the second case, two nodes on orthogonal sides of
...
We argue that the non-dominated macro-edges computed using our strategy are both
...
%We claim this method preserves optimality when traversing arbitrary empty rectangles in an 8-connected grid map.
\begin{lemma} \label{lemma-rooms} Let $R$ be an empty rectangle in
an 8-connected grid map.  Further, let $m$ and $n$ be two locations along its
perimeter.  Then, $m$ and $n$ can be connected optimally through a path that
mentions only nodes on the perimeter of $R$ and possibly involves one macro
edge.  
\end{lemma}
...
follows: if $m$ and $n$ are not connected by a macro edge we can walk along the
perimeter from $m$ to the first intermediate node $m'$ which has a macro-edge
incident with $n$.  Since $m'$ is the first such node, the weight of the
macro-edge $(m', n)$ is equal to the diagonal distance between $m'$ and $n$.  It
is now easy to show that the length of the resultant path ${m, m', n}$ is equal
to the ocnode distance between $m$ and $n$ and therefore optimal.
\end{proof}
...
%\par \noindent
%\newline
We claim that the symmetry elimination procedure outlined in this section 
preserves the optimality of solutions:
\begin{theorem}
