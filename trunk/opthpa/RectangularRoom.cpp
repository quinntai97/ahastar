...
RectangularRoom::RectangularRoom(const int x, const int y, bool perimeterReduction_, bool bfReduction_) 
	throw(std::invalid_argument)
	: HPACluster(x, y, DUMMYDIMENSION, DUMMYDIMENSION),
	  perimeterReduction(perimeterReduction_), bfReduction(bfReduction_)
{
	if(x < 0 || y <0)
		throw std::invalid_argument("RectangularRoom: x,y coordinates must be >= 0");
...
void RectangularRoom::addNodesToCluster(HPAClusterAbstraction* aMap, int** clearance)
	throw(std::invalid_argument)
{
	if(aMap == NULL)
		throw std::invalid_argument("RectangularRoom::addNodesToCluster cluster"
	...
	for(int x=getHOrigin(); x<getHOrigin()+getWidth(); x++)
	{
		for(int y=getVOrigin(); y<getVOrigin()+getHeight(); y++)
		{
			...
			{
				throw std::invalid_argument("RectangularRoom: tried to add to "
...
void 
RectangularRoom::buildEntrances(HPAClusterAbstraction* hpamap) 
	throw(std::invalid_argument)
{
	if(getVerbose())
	{
...
void RectangularRoom::addNodesToCluster(HPAClusterAbstraction* aMap)
	throw(std::invalid_argument)
{
	if(aMap == NULL)
		throw std::invalid_argument("RectangularRoom::addNodesToCluster cluster"
...
void RectangularRoom::frameCluster(HPAClusterAbstraction* aMap)
{
        if(getVerbose())
...
void RectangularRoom::addCardinalMacroEdges(HPAClusterAbstraction *aMap)
{
//	this->print(std::cout);
//	std::cout << std::endl;
	if(getVerbose())
...
void
RectangularRoom::addDiagonalMacroEdges(HPAClusterAbstraction* aMap)
{
	graph* absg = aMap->getAbstractGraph(1);
	macro = 0;
...
void
RectangularRoom::addDiagonalFanMacroEdges(HPAClusterAbstraction* aMap)
{
	graph* absg = aMap->getAbstractGraph(1);
	macro = 0;
	...
	for(int fx=this->getHOrigin()+1; // skip corner tiles (equivalent edges added during ::addDiagonalMacroEdges)
			fx<this->getHOrigin()+this->getWidth()-1;  // skip corner tiles
			fx++)
	{
		int fy = this->getVOrigin();
		node* first = absg->getNode(
	...
	// side of the cluster
	for(int fy=this->getVOrigin()+1; // skip corner tiles
			fy<this->getVOrigin()+this->getHeight()-1; // skip corner tiles
			fy++)
	{
...
void RectangularRoom::addMacroEdges(HPAClusterAbstraction *aMap)
{
	if(getVerbose())
	{
...
void RectangularRoom::extend(HPAClusterAbstraction* aMap)
{
	setWidth(1);
	setHeight(1);
...
void RectangularRoom::extend(HPAClusterAbstraction* aMap, int** clearance)
{
...
void RectangularRoom::initOpenGLCoordinates(HPAClusterAbstraction* aMap)
{
	Map* map = aMap->getMap();
...
void RectangularRoom::openGLDraw()
{
...
void RectangularRoom::connectParent(node*, HPAClusterAbstraction*)
{
}
...
void RectangularRoom::processHorizontalEntrance(
...
void RectangularRoom::buildHorizontalEntrances(HPAClusterAbstraction* hpamap)
{
	if(getVerbose())
...
void RectangularRoom::processVerticalEntrance(
		HPAClusterAbstraction* hpamap, int x, int y, int length)
{
	if(getVerbose())
	{
...
void RectangularRoom::buildVerticalEntrances(HPAClusterAbstraction* hpamap)
{
	if(getVerbose())
		std::cout << "buildHorizontalEntrances"<<std::endl;
...
bool RectangularRoom::canExtendClearanceSquare(HPAClusterAbstraction* hpamap)
{
...
bool RectangularRoom::canExtendHorizontally(HPAClusterAbstraction* hpamap)
{
	int x = getHOrigin() + getWidth();
	for(int y = getVOrigin(); y<getVOrigin() + getHeight(); y++)
	{
...
bool RectangularRoom::canExtendVertically(HPAClusterAbstraction* hpamap)
{
...
// coordinate (x, y)
int RectangularRoom::findHorizontalLength(int x, int y, 
...
// coordinate (x, y)
int RectangularRoom::findVerticalLength(int x, int y, 
		HPAClusterAbstraction* hpamap)
{
	int length = 0;
	for(int yprime = y; 
...
MacroNode*
RectangularRoom::nextNodeInRow(int x, int y, HPAClusterAbstraction* hpamap,
		bool leftToRight)
{
	MacroNode* retVal = 0;
...
MacroNode*
RectangularRoom::nextNodeInColumn(int x, int y, HPAClusterAbstraction* hpamap,
		bool topToBottom)
{
	MacroNode* retVal = 0;
	graph* absg = hpamap->getAbstractGraph(1);
...
void RectangularRoom::addParent(node *n, HPAClusterAbstraction* hpamap) 
	throw(std::invalid_argument)
{
	assert(n->getLabelL(kParent) == -1);
