...
In order to compute clearances we employ the technique described by \citeauthor{harabor08}~\shortcite{harabor08}.
The algorithm can be summarised as follows:
For each tile $t$ on the map, compute a maximal size square bounding box that contains no obstacles
and has $t$ as the upper-left corner.
The length of the bounding box becomes the clearance value associated with the tile.
%\input alg_clearance
\par
We introduce the following
procedure to construct empty rectangular rooms once clearance values are computed:
