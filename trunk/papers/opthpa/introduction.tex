...
Despite this, many tiles have \emph{f-values} smaller than the goal's, and therefore A* must expand them.
...
In order to solve the problem more efficiently we will automatically divide the map into a set of obstacle-free rectangular rooms and 
...
%Infact, if we could traverse directly from one side of an empty room to another there appears to \emph{always}
%exist an optimal length solution that involves only nodes from the perimeter.
...
Cases when the start (or the goal) location belongs to the interior of an empty rectangle are handled as detailed in the third section.
\par
...
Our experimental results show the average number of A* node expansions is reduced by between 29-65\%. 
