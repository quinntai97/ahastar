...
%nodes on the perimeter of each empty rectangular room. 
...
of a search.  
%Our method is an adaptation of \citeauthor{harabor10}'s
%re-insertion algorithm that specifically deals with 8-connected grids.  
...
%location to any node on the perimeter of the start or goal room. 
%\par \noindent
%\newline
...
%It is based on the observation that for every segment of an optimal length path, which enters a room at some node $m$,
%traverses through the interior of the room and exits at some node $n$, there is an equivalent length path that mentions only 
